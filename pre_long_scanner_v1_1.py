...
def get_json(path, params):
    ...
    for attempt in range(RETRIES):
        # Global pacing prevents the worker threads from bursting the
        # Bitget public API. The old V1 could produce HTTP 429 with 8 workers.
        with _rate_lock:
            now = time.monotonic()
        ...
        try:
            req = Request(url, headers={"User-Agent": "bitget-pre-long-v1.1/1.0"})

...
def get_candles(symbol, timeframe, limit=160):
    data = get("/api/v2/mix/market/candles", {
        ...
        "granularity": timeframe, "limit": str(limit)
    })
...
def classify_setup(ma20_dist, rsi, vol_ratio, breakout, near_breakout,
                    h_alignment, h_partial, ma20_up, change24h):
    if ma20_dist > MAX_MA20_DISTANCE or rsi >= 75 or change24h >= 12:
        return "🔴 추격 금지", "MA20 이격/RSI/24H 급등 중 하나가 과열 기준 초과"

    if (h_alignment and breakout and ma20_dist <= 0.04
            and 50 <= rsi <= 70 and vol_ratio >= MIN_VOL_RATIO):
        return "🟢 진입 가능", "추세 + 돌파 + 거래량 확인"

    if (h_alignment and ma20_up and ma20_dist <= 0.035
            and 45 <= rsi <= 68):
        return "🟡 눌림 대기", "상승 추세 유지, MA20 부근 눌림 확인"

    if h_alignment and near_breakout and 45 <= rsi <= 70:
        return "🔵 돌파 대기", "20봉 고점 돌파 확인 필요"

    if h_partial and ma20_up and 45 <= rsi <= 68:
        return "🟡 눌림 대기", "4H 추세는 양호하나 완전 정배열 전"

    return "⚪ 관찰", "핵심 진입 조건 일부 미충족"
...
def analyze(symbol, df1h, df4h, ticker):
    ...
    if any(pd.isna(v) for v in [ma20, ma60, ma120, h20, h60, h120, x.rsi14, x.atr14]):
        return None
    ...
    near_breakout = bool(not breakout and price >= recent_high_20 * 0.99)
    ...
    h_partial = bool(
        h.close > h20 and h20 > h60 and h20 > hprev.ma20
    )

    classification, setup_reason = classify_setup(
        ma20_dist, rsi, vol_ratio, breakout, near_breakout,
        h_alignment, h_partial, ma20_up, change24h
    )
    ...
    target_1 = float(entry_reference + 2 * risk) if risk > 0 else float(entry_reference)
    stop_distance_pct = risk / entry_reference * 100 if entry_reference > 0 else 0.0
...
def main():
    ...
    print("v2.2: 진입/눌림/돌파/추격 구분 + 기준 손절/2R 목표를 계산합니다.\n")
    ...
    class_rank = {"🟢 진입 가능":0, "🟡 눌림 대기":1, "🔵 돌파 대기":2, "⚪ 관찰":3, "🔴 추격 금지":4}
    out["class_rank"] = out["classification"].map(class_rank).fillna(9)
    out = out.sort_values(
        ["class_rank", "score", "4h_alignment", "vol_ratio", "turnover_24h"],
    ...
    cols = [
        "symbol","score","classification","setup_reason","price",
        "entry_reference","reference_stop","target_1_2R","stop_distance_pct",
        "ma20_dist_pct","vol_ratio","turnover_24h","rsi14",
        "breakout_20h","near_breakout","4h_alignment","24h_change_pct"
    ]
    ...
    out.drop(columns=["class_rank"]).to_csv("bitget_1h_long_candidates_v2_2.csv", index=False)
